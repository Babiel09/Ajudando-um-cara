...
import { ShowAll, Delete, InsertIntoDB, ShowOne, Update } from "../services/itesnServices";

export abstract class itensController{
    ...
    try{
        const {nome, price} = req.body as {nome:string, price:number};
        if(!nome){
            throw new Error("Você precisa passar o nome!");
        };
        const postedBingo = await inserir.execute({nome, price});
    ...
    static async put(req:FastifyRequest, reply:FastifyReply){
        ...
        try{
            ...
            const updetedBingo = await atualizar.execute({id,nome, price});
