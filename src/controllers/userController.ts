import { Usereses } from "../services/userService";
import { InsertIntoDB, Delete, ShowAll, ShowOne, Update } from "../services/userService";
...
export abstract class userController{
    ...
    try{
        const {nome, wins, coins, email, pass, role} = req.body as {nome:string, wins:number, coins:number, email:string, pass:string, role:Usereses};
        if(!nome){
            throw new Error("Você precisa passar o nome!");
        };
        const postedBingo = await inserir.execute({nome, wins, coins, email, pass, role});
    ...
    static async put(req:FastifyRequest, reply:FastifyReply){
        ...
        try{
            ...
            const updetedBingo = await atualizar.execute({id,nome, wins, coins, email, pass, role});
